#!/usr/bin/env python3
"""
Traffic generation script for Stage 5 (with Database Backend).
...
FRONTEND_URL = "http://localhost:8111"  # Frontend for stage5
...
resource = Resource(attributes={"service.name": "traffic-generator-stage5"})
...
async def main():
    ...
    logging.info(
        f"Starting traffic generation for Stage 5: {NUM_USERS} users, "
    ...
    logging.info(
        f"Architecture: Traffic Generator → Frontend → Dice Roller → Die Service → PostgreSQL"
    )
    ...
    try:
        ...
        logging.info(
            "Make sure Stage 5 is running: cd progressive/stage5 && docker compose up -d"
        )
        return
    except Exception as e:
    ...
    logging.info(
        "  - Sync traces: traffic-generator → frontend → dice-roller → die-service → postgres"
    )
    logging.info(
        "  - Async traces: Look for traces with concurrent child spans in dice-roller service"
    )
    logging.info(
        "  - Database spans: Look for PostgreSQL query spans in die-service traces"
    )
